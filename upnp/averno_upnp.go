...
func AllUpnpOpen() {
	...
	if err = mapping_http.AddPortMapping(ClusterPort, ClusterPort, "TCP"); err == nil {
		...
		log.Printf("[UPnP] No UPnP on TCP %d: network UPnP-agnostic, %s", ClusterPort, err.Error())
	}
	...
	if err = mapping_http.AddPortMapping(ClusterPort, ClusterPort, "UDP"); err == nil {
		...
		log.Printf("[UPnP] No UPnP on UDP %d: network UPnP-agnostic, %s", ClusterPort, err.Error())
	}
